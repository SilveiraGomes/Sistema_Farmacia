...
const ACTION_ACCESS = Object.freeze({
  ...
  "profiles.summaries": "read", "profiles.list": "read", "profiles.permissions": "read",
  "profiles.updatePermissions": "write",
  "fornecedores.list": "read", "fornecedores.create": "write",
  "fornecedores.update": "write", "fornecedores.toggle": "write",
  "estoque.addLot": "write", "estoque.deduct": "write", "estoque.getLotes": "read",
  "estoque.updatePrice": "write", "estoque.listPrices": "read", "estoque.listProducts": "read",
  "estoque.createProduct": "write", "estoque.updateProduct": "write",
  "estoque.deleteProduct": "write", "estoque.getProduct": "read",
  "estoque.listCategories": "read", "estoque.createCategory": "write",
  "estoque.listSubcategories": "read", "estoque.createSubcategory": "write",
  "estoque.updateCategory": "write", "estoque.deleteCategory": "write",
  "estoque.updateSubcategory": "write", "estoque.deleteSubcategory": "write",
  "estoque.importCategories": "write", "estoque.importSubcategories": "write",
  "estoque.importProducts": "write",
  "clientes.list": "read", "clientes.getById": "read", "clientes.create": "write",
  "clientes.update": "write", "clientes.delete": "write",
  "vendas.create": "write", "vendas.recentDocuments": "read", "vendas.listDocuments": "read",
  "vendas.cancelDocument": "write", "vendas.convertProforma": "write",
  "compras.list": "read", "compras.create": "write", "compras.updateStatus": "write",
  "compras.receive": "write",
  "financeiro.contasPagar": "read", "financeiro.marcarPago": "write",
  "financeiro.list": "read", "financeiro.overview": "read", "financeiro.create": "write",
  "financeiro.delete": "write",
  "relatorio.data": "read", "relatorio.rawData": "read",
  "backup.manual": "read", "backup.list": "read", "backup.restore": "write",
  "backup.delete": "write", "backup.integrityCheck": "read", "backup.serviceStatus": "read",
  "backup.openFolder": "read", "backup.chooseLocation": "read", "backup.create": "read",
  "backup.selectFile": "read",
  "alerts.getSystemAlerts": "read", "dashboard.metrics": "read",
  "operation.state": "read", "operation.openDay": "write", "operation.closeDay": "write",
  "operation.openShift": "write", "operation.closeShift": "write",
  "configuration.snapshot": "read", "configuration.updateSection": "write",
  "configuration.importLegacy": "write", "configuration.document.reserveNumber": "write",
  "configuration.catalog.create": "write", "configuration.catalog.update": "write",
  "configuration.catalog.reorder": "write", "configuration.catalog.activate": "write",
  "configuration.catalog.deactivate": "write",
  "reports.sync.status": "read", "reports.sync.now": "read", "reports.sync.history": "read",
  "reports.enqueue": "write",
  "invoice.savePDF": "read", "heldSales.load": "read", "heldSales.save": "write",
  "heldSales.clear": "write", "printing.listPrinters": "read", "invoice.print": "read",
  "report.savePDF": "read", "report.print": "read", "window.setFullscreen": "read",
  "window.isFullscreen": "read", "window.close": "read",
  "license.status": "license", "license.activate": "license", "license.validate": "license",
  "license.machineId": "license",
});
const LEGACY_ACTION_ACCESS = Object.freeze({
  getProducts: "read",
  addProduct: "write",
  getCategories: "read",
  addCategory: "write",
  addSubcategory: "write",
});

function classifyRoutes(routes, classifications = ACTION_ACCESS) {
  const routeNames = Object.keys(routes).sort();
  const classifiedNames = Object.keys(classifications).sort();
  const missing = routeNames.filter((name) => !classifications[name]);
  const stale = classifiedNames.filter((name) => !routes[name]);
  if (missing.length || stale.length) {
    throw new Error(`IPC route classification mismatch; missing=${missing.join(",")}; stale=${stale.join(",")}`);
  }
  for (const access of Object.values(classifications)) {
    if (!["read", "write", "license"].includes(access)) {
      throw new Error(`Invalid IPC route classification: ${access}`);
    }
  }
...
async function getCurrentSession(dependencies) {
  ...
  if (!session || !session.user || !session.user.id) {
    if (dependencies.licenseService?.status?.()?.canWrite === true) {
...
function buildRouteMap(overrides = {}) {
  ...
  const routes = {
    ...
    "license.status": () => dependencies.licenseService?.status() ??
      ({ state: "unactivated", canWrite: false, readOnly: true }),
    "license.activate": (data = {}) => dependencies.licenseService.activate(data.licenseKey),
    "license.validate": (data = {}) => dependencies.licenseService.validate(data.licenseKey),
    "license.machineId": () => ({ machineId: dependencies.licenseService.machineId() }),
  };
  classifyRoutes(routes);
  return Object.fromEntries(Object.entries(routes).map(([action, route]) => [
    action,
    ACTION_ACCESS[action] === "write"
      ? async (...args) => {
          if (typeof dependencies.licenseService?.assertWriteAllowed === "function") {
            dependencies.licenseService.assertWriteAllowed();
          } else {
            const error = new Error("Serviço de licenças indisponível.");
            error.code = "LICENSE_UNAVAILABLE";
            throw error;
          }
          return route(...args);
        }
...
function registerLegacyRoutes(
  ...
  ipc.on("toMain", async (event, args) => {
    const legacyAccess = LEGACY_ACTION_ACCESS[args?.action];
    if (legacyAccess === "write") {
      try {
        ...
        replyLegacyError(event, `${args.action}Response`, error, "Licença sem escrita:");
        return;
      }
    }
...
async function init(models, options = {}) {
  ...
  const dependencies = {
    ...
    ...(options.licenseService ? { licenseService: options.licenseService } : {}),
  };
