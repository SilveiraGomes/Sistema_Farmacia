...
const catalogOptions = (items, system) => items.map((item, index) => {
  const [code, name] = Array.isArray(item) ? item : [item, titleFromCode(item)];
  return { code, name, order: index, system };
});
...
const technical = (...items) => ({
  editable: false,
  options: catalogOptions(items, true),
});
...
const SETTING_DEFINITIONS = deepFreeze({
  ...
  'documents.headerText': { group: 'documents', type: 'text', defaultValue: 'Sistema de Farmácia' },
...
const CATALOG_DEFINITIONS = deepFreeze({
  ...
  document_types: technical('factura', 'factura_recibo', 'recibo', 'proforma', ['credito', 'Crédito'], ['nota-credito', 'Nota de crédito']),
  document_statuses: technical('emitido', 'pago', 'pendente', 'anulado', 'convertido'),
  financial_entry_types: technical(['expense', 'Despesa'], ['revenue', 'Receita'], ['loss', 'Perda']),
