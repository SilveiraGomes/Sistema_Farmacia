...
const menuItems = [
  ...
  { id: 'relatorios', label: 'Relatórios', icon: BarChart3, permission: 'relatorios.ver' },
  { id: 'documentos', label: 'Documentos', icon: FileText, permission: 'documentos.ver' },
  { id: 'configuracoes', label: 'Configurações', icon: Settings, permission: 'configuracoes.ver' },
  { id: 'usuarios', label: 'Usuários', icon: UserRound, permission: 'usuarios.ver' },
];
const secondaryMenuIds = new Set(['configuracoes', 'usuarios']);

function Navbar({ currentView, hasPermission, isCollapsed, setCurrentView, toggleCollapsed }) {
  const canView = typeof hasPermission === 'function' ? hasPermission : () => true;
  ...
  const secondaryItems = menuItems
    .filter((item) => secondaryMenuIds.has(item.id))
    .filter((item) => canView(item.permission));
  const shouldShowDivider = primaryItems.length > 0 && secondaryItems.length > 0;
      ...
      <nav className="side-menu" aria-label="Menu principal">
        {primaryItems.map((item) => (
          <button
            key={item.id}
            type="button"
            className={currentView === item.id ? 'side-link active' : 'side-link'}
            onClick={() => setCurrentView(item.id)}
          >
            <span className="side-icon"><item.icon size={23} strokeWidth={2.1} /></span>
