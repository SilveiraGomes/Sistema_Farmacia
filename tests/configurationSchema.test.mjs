...
test("syncDatabaseSchema creates and returns configuration models without losing data", async () => {
  const userDataPath = await mkdtemp(
    join(tmpdir(), "pharmacy-configuration-schema-"),
  );
  ...
  try {
    ...
    for (const table of [
      ...
      "OpcoesCatalogo",
      "AuditoriasConfiguracao",
    ]) {
      assert.ok(tables.includes(table), `${table} should exist`);
    }

    const { ConfiguracaoSistema, OpcaoCatalogo, AuditoriaConfiguracao } =
      getModels();
    ...
    assert.equal(
      await ConfiguracaoSistema.count({ where: { chave: "empresa.nome" } }),
      1,
    );
  } finally {
    await db.close();
...
test("OpcoesCatalogo enforces unique catalogo and codigo pairs", async () => {
  const userDataPath = await mkdtemp(
    join(tmpdir(), "pharmacy-catalog-schema-"),
  );
  const db = await connectDB(fakeAppFor(userDataPath), "development");
  ...
  try {
    ...
    await assert.rejects(
      () =>
        OpcaoCatalogo.create({
          catalogo: "unidades",
          codigo: "cx",
          nome: "Caixa duplicada",
        }),
      (error) => error.name === "SequelizeUniqueConstraintError",
    );
    await OpcaoCatalogo.create({
      catalogo: "formas_pagamento",
      codigo: "cx",
      nome: "Caixa",
    });
  } finally {
...
test("syncDatabaseSchema can normalize legacy OpcoesCatalogo unique catalogo indexes for SQLite", async () => {
  const userDataPath = await mkdtemp(
    join(tmpdir(), "pharmacy-legacy-catalog-schema-"),
  );
  const db = await connectDB(fakeAppFor(userDataPath), "development");
  ...
  try {
    ...
    await db.query(
      "CREATE UNIQUE INDEX unique_catalogo ON OpcoesCatalogo(catalogo)",
    );
    ...
    assert.equal(
      await OpcaoCatalogo.count({ where: { catalogo: "unidades" } }),
      2,
    );
  } finally {
    await db.close();
...
test("connectDB uses the packaged userData SQLite database in production", async () => {
  const userDataPath = await mkdtemp(
    join(tmpdir(), "pharmacy-production-database-"),
  );
