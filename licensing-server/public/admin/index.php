...
try {
    if ($method === 'POST') {
        ...
        if ($action === 'correct_plan') { requireAction($admin,'correct_plan'); $result=$service->correctPlan((int)$_POST['license_id'],(string)$_POST['plan'],actor($admin)); $_SESSION['issued_key']=flashKey($result['licenseKey'],$csrf); redirectAdmin('issued'); }
...
<?php else: $rows=$service->licenses((string)($_GET['q']??''));?><section class="card"><h1>Licenças</h1><form method="get"><input type="hidden" name="page" value="licenses"><input name="q" value="<?=h($_GET['q']??'')?>" placeholder="Cliente, ID ou estado"><button>Pesquisar</button></form><table><tr><th>Cliente</th><th>Plano/estado</th><th>Validade</th><th>Máquina</th><th>Ações</th></tr><?php foreach($rows as$r):?><tr><td><?=h($r['customer_name'])?><br><small><?=h($r['public_id'])?></small></td><td><?=h($r['plan'].' / '.$r['status'])?></td><td><?=h($r['expires_at'])?></td><td><?=h($r['machine_hash']?'vinculada':'livre')?></td><td><a href="?page=history&amp;license_id=<?=h($r['id'])?>">Histórico</a> <?php $labels=['renew'=>'renew','transfer'=>'transfer','correct_plan'=>'Corrigir plano','block'=>'block','revoke'=>'revoke']; foreach(['renew','transfer','correct_plan','block','revoke'] as$a):if(!AdminAuth::can($admin['role'],$a))continue;if($a==='renew'&&$r['plan']==='demo')continue;if($a==='correct_plan'&&($r['plan']==='demo'||$r['status']==='revoked'))continue;?><form method="post" style="display:inline"><input type="hidden" name="csrf" value="<?=h($csrf)?>"><input type="hidden" name="action" value="<?=h($a)?>"><input type="hidden" name="license_id" value="<?=h($r['id'])?>"><?php if($a==='renew'):?><select name="plan"><option value="one_year">+1 ano</option><option value="two_years">+2 anos</option><option value="three_years">+3 anos</option></select><?php endif?><?php if($a==='correct_plan'):?><select name="plan" title="Revoga esta licença e emite de imediato uma nova com o plano escolhido, para o mesmo cliente."><option value="one_year"<?=$r['plan']==='one_year'?' disabled':''?>>1 ano</option><option value="two_years"<?=$r['plan']==='two_years'?' disabled':''?>>2 anos</option><option value="three_years"<?=$r['plan']==='three_years'?' disabled':''?>>3 anos</option></select><?php endif?><button class="<?=in_array($a,['block','revoke'],true)?'danger':''?>"><?=h($labels[$a])?></button></form><?php endforeach?></td></tr><?php endforeach?></table></section><?php endif?></main></body></html>
